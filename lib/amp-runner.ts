...
export interface AmpEvalOptions {
  timeout?: number;
  verbose?: boolean;
  debug?: boolean;
  apiKey?: string;
...
export class AmpRunner {
  ...
  constructor(options: AmpEvalOptions = {}) {
    this.verbose = options.verbose || false;
    this.debug = options.debug || false;
    this.apiKey = options.apiKey || process.env.AMP_API_KEY;
  ...
  async runAmpEval(
    ...
    try {
      ...
      process.stdout.write(`🚀 Running Amp...`);

      if (this.verbose) {
        console.log(`\n🚀 Running Amp on ${outputDir}...`);
        console.log(`📝 Prompt: ${prompt}`);
        console.log('─'.repeat(80));
      }

      // Run Amp with the prompt
      const ampResult = await this.executeAmp(outputDir, prompt, timeout);

      // Clear progress indicator
      if (!this.verbose) {
        process.stdout.write(`\r🚀 Running Amp... ✅\n`);
      }

      if (!ampResult.success) {
        return {
          success: false,
          output: ampResult.output,
          error: ampResult.error,
  ...
  private async executeAmp(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.apiKey) {
        env.AMP_API_KEY = this.apiKey;
      }

      // Spawn Amp process with --execute flag for non-interactive mode
      // --dangerously-allow-all: bypass tool approval prompts for automation
      const args = [
      ...
      if (this.verbose) {
        console.log('🚀 Spawning amp process with:');
        console.log('  Command: amp');
      ...
      this.processes.set(processId, ampProcess);

      // Close stdin immediately so Amp doesn't wait for input
      if (ampProcess.stdin) {
      ...
      ampProcess.stderr?.on('data', (data) => {
        const output = data.toString();
        if (this.verbose) {
          console.log('⚠️  Amp stderr:', output);
        }
        stderr += output;
      });

      const timeoutId = setTimeout(() => {
        ampProcess.kill('SIGTERM');
        setTimeout(() => {
          ampProcess.kill('SIGKILL');
        }, 5000);
        resolve({
          success: false,
          output: stdout,
          error: `Amp process timed out after ${timeout}ms`
        });
      }, timeout);

      ampProcess.on('exit', (code, signal) => {
        clearTimeout(timeoutId);
        this.processes.delete(processId);

        if (this.verbose) {
          console.log('─'.repeat(80));
          console.log(`Amp finished with code: ${code}, signal: ${signal}`);
        }
        ...
        if (signal) {
          resolve({
            success: false,
            output: stdout,
            error: `Amp process killed by signal ${signal}`
          });
        } else if (code === 0) {
          resolve({
            success: true,
          ...
          resolve({
            success: false,
            output: stdout,
            error: stderr || `Amp process exited with code ${code}`
          });
        }
      });
...
export async function runAmpEval(
  evalPath: string,
  options: AmpEvalOptions = {},
  ...
  if (useWorktree) {
    ...
    outputDir = path.join(worktreePath, "output-amp");
  } else {
    worktreeInputDir = inputDir;
    outputDir = path.join(fullEvalPath, "output-amp");
  }

  const runner = new AmpRunner(options);

  try {
    const result = await runner.runAmpEval(worktreeInputDir, outputDir, prompt, evalPath, options.timeout);
    return result;
  } finally {
